...
salary_player<-dat[, c('salary','player')]
salary_player
mean(dat$salary)
...
nrow(dat[dat$player,])  
mean(dat$salary)
dat1<-arrange(dat,desc(salary))##441
...
fouls=dat2$fouls
efficiency=points+rebounds+assist+steals+blocks-missed_fg-
  missed_ft-turnovers
min_per_game=dat2$minutes/dat2$games_played
position=dat3$position
pts_per_game=total_points/dat2$games_played

data2<-mutate(data1,missed_fg,missed_ft,total_points,rebounds,assist,steals,blocks,turnovers,fouls,efficiency,min_per_game,pts_per_game,salary, position)
```

```{r}
ggplot(data = data2, 
       aes(x = total_points, y = salary)) +
  geom_point(aes(color= position),alpha=0.9) +
  facet_wrap(~ position)+
  geom_smooth(method = loess)+
  ggtitle("total points vs salary")
```
...
```
##It shows a week correlation between efficiency and salary.
```{r}
